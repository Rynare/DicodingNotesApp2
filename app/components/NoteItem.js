...
export class NoteItem extends HTMLElement {
  ...
  render() {
    ...
    getNoteById(this.id).then((result) => {
      ...
      note_createAt.innerText = formatDate(result.data.createdAt).replace(
        "pukul",
        " | ",
      );
  ...
  runOptionEvent(element) {
    element.addEventListener("note-option-changed", (event) => {
      switch (event.detail.noteOption) {
        ...
        case "unarchive":
          if (
            confirm("Yakin ingin mengeluarkan note/catatan ini dari arsip?")
          ) {
            setUnarchiveNote(this.id)
              .then((result) => {
                if ((result.status = "success")) {
                  runSwal({
                    type: "success",
